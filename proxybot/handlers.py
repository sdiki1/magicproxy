...
PROXY_FOOTER = "👀Услуга сервиса  @wprx_bot"
...
def build_welcome_text() -> str:
    return (
        "🚀 Автоматический сервис по приобретению прокси для веб-скрейпинга, маркетинговых исследований или обеспечения анонимности клиентов. 🌐\n\n"
...
def create_router(
    ...
    async def build_payment_message(
        *,
        ...
        pay_step = "1) Нажмите «⭐️ Оплатить звёздами»"
        activate_step = "2) После оплаты прокси выдаются автоматически."
        if has_yookassa:
            pay_step = "1) Нажмите «Оплатить через ЮKassa» или «⭐️ Оплатить звёздами»"
            activate_step = "2) После оплаты нажмите «Активировать»."
        return (
            f"{tg_emoji(EMOJI_SHIELD, '🛡')} <b>Платёж создан</b>\n\n"
            f"Срок: <b>{months_count} {month_word(months_count)}</b>\n"
    ...
    async def cb_buy_target(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        try:
            ...
            await edit_or_send(
                callback,
                text=f"Не удалось создать платёж в ЮKassa.\n{exc}",
                reply_markup=main_menu_keyboard(),
                parse_mode=None,
            )
    ...
    async def state_waiting_friend_tg_id(message: Message, state: FSMContext) -> None:
        ...
        if message.user_shared is not None:
            ...
            if payload.strip().lower() in {"отмена", "отмена выбора", "cancel"}:
                await state.clear()
                await message.answer("Выбор пользователя отменён.", reply_markup=ReplyKeyboardRemove())
                await message.answer(build_welcome_text(), reply_markup=main_menu_keyboard())
                return
        ...
        try:
            ...
            await message.answer(
                f"Не удалось создать платёж в ЮKassa.\n{exc}",
                reply_markup=ReplyKeyboardRemove(),
            )
            await message.answer(build_welcome_text(), reply_markup=main_menu_keyboard())
            return
    ...
    async def cb_pay_stars(callback: CallbackQuery) -> None:
        ...
        try:
            ...
            await callback.answer("Не удалось отправить счёт в звёздах", show_alert=True)
            return

        await callback.answer("Счёт в звёздах отправлен")
    ...
    async def pre_checkout(pre_checkout_query: PreCheckoutQuery) -> None:
        try:
            ...
            if pre_checkout_query.currency != "XTR":
                await pre_checkout_query.answer(ok=False, error_message="Поддерживается только оплата звёздами.")
                return
            ...
            try:
                await pre_checkout_query.answer(
                    ok=False,
                    error_message="Не удалось проверить платёж. Попробуйте ещё раз.",
                )
            except TelegramBadRequest:
                pass
    ...
    async def successful_payment(message: Message) -> None:
        ...
        payment_id_raw = payload.split(":", maxsplit=1)[1]
        if not payment_id_raw.isdigit():
            await message.answer("Некорректный payload звёздного платежа.")
            return
        ...
        if recipient_profile.tg_user_id == message.from_user.id:
            ...
            await message.answer("Звёздный платёж подтверждён, прокси выданы.")
            return

        delivery_text = (
            f"Звёздный платёж подтверждён.\n"
    ...
    async def cb_cancel_payment(callback: CallbackQuery) -> None:
        ...
        if not yookassa_payment_id:
            await callback.answer(
                "Для платежа звёздами отмена недоступна. Просто не оплачивайте счёт.",
                show_alert=True,
            )
            return
        if yookassa_payment_id:
            ...
            if remote_status == "succeeded":
                await callback.answer("Платёж уже оплачен. Нажмите «Активировать».", show_alert=True)
                return
    ...
    async def cb_pay(callback: CallbackQuery) -> None:
        ...
        if not yookassa_payment_id:
            await callback.answer(
                "Этот платёж не через ЮKassa. Оплатите звёздами ⭐️ (кнопка выше), выдача пройдёт автоматически.",
                show_alert=True,
            )
            return
        ...
        if remote_status != "succeeded":
            ...
            if confirmation_url and callback.message is not None:
                try:
                    ...
                    pass
            await callback.answer("Платёж пока не завершён", show_alert=True)
            return
        ...
        try:
            ...
            delivery_text = (
                ...
                "Пусть пользователь запустит бота командой /start и нажмёт «Мои прокси»."
            )
        await edit_or_send(
            callback,
            text=delivery_text,
