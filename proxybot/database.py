...
from .proxy_pool_file import apply_pool_credential_updates
...
def generate_proxy_credentials(port: int) -> tuple[str, str]:
    return f"u{port}_{secrets.token_hex(4)}", secrets.token_urlsafe(18)


class Database:
    def __init__(self, path: str, proxy_pool_file: str | None = None):
        self.path = path
    ...
    def _sync_pool_credentials_file(self, updates: list[dict[str, Any]]) -> None:
        if not updates:
            return
        try:
            apply_pool_credential_updates(self.proxy_pool_file, updates)
        except Exception:
            logger.exception("Failed to sync rotated proxy credentials to pool file")
    ...
    async def revoke_proxy_link_for_user(self, user_id: int, proxy_link_id: int) -> bool:
        timestamp = now_ts()
        rotated_credentials: list[dict[str, Any]] = []
        await self.conn.execute("BEGIN IMMEDIATE")
        try:
            cursor = await self.conn.execute(
                """
            ...
            if pool_row is not None:
                port = int(pool_row["port"])
                new_username, new_password = generate_proxy_credentials(port)
                await self.conn.execute(
                    """
                    UPDATE proxy_pool
                    SET status = 'free', assigned_link_id = NULL, username = ?, password = ?, updated_at = ?
                    WHERE assigned_link_id = ?
                    """,
                    (new_username, new_password, timestamp, proxy_link_id),
                )
                rotated_credentials.append(
                    {
                        "port": port,
            ...
            await self.conn.commit()
            self._sync_pool_credentials_file(rotated_credentials)
            return True
        except Exception:
            await self.conn.rollback()
            raise
    ...
    async def revoke_all_active_links_for_user(self, user_id: int) -> int:
        ...
        try:
            ...
            cursor = await self.conn.execute(
                f"""
                SELECT assigned_link_id, port, username
                FROM proxy_pool
            ...
            for pool_row in pool_rows:
                assigned_link_id = int(pool_row["assigned_link_id"])
                port = int(pool_row["port"])
                new_username, new_password = generate_proxy_credentials(port)
                await self.conn.execute(
                    """
                    ...
                    WHERE assigned_link_id = ?
                    """,
                    (new_username, new_password, timestamp, assigned_link_id),
                )
                rotated_credentials.append(
                    {
                        "port": port,
    ...
    async def expire_due(self) -> None:
        ...
        cursor = await self.conn.execute(
            """
            SELECT pp.assigned_link_id, pp.port, pp.username
            FROM proxy_pool pp
            JOIN proxy_links pl ON pl.id = pp.assigned_link_id
            WHERE pl.status = 'expired'
            """,
        )
        pool_rows = await cursor.fetchall()
        await cursor.close()
